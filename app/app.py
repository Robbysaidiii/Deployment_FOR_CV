'''
    Contoh Deployment untuk Domain Computer Vision (CV)
...
def apiDeteksi():
    ...
    if filename != '':
        ...
        if file_ext in app.config['UPLOAD_EXTENSIONS']:
            ...
            image_array = np.array(test_image_resized) / 255.0  # Normalisasi antara 0 dan 1
            test_image_x = np.expand_dims(image_array, axis=0)  # Tambahkan dimensi batch
            
            # Log input untuk debugging
            logging.info(f"Shape input image array: {test_image_x.shape}")
            
            # Prediksi Gambar
            y_pred_test_single = model.predict(test_image_x)
            
            # Log output prediksi untuk debugging
            logging.info(f"Predicted probabilities: {y_pred_test_single}")
            ...
            # Log hasil prediksi akhir
            logging.info(f"Predicted class index: {y_pred_test_classes_single[0]}, Predicted label: {hasil_prediksi}")
